            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full h-11 px-3.5 pr-11 rounded-xl border bg-white text-gray-900 text-sm placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:border-transparent transition ${
                    pwTouched && !isPasswordValid(password)
                      ? 'border-red-300 focus:ring-red-400/40'
                      : 'border-green-200 focus:ring-emerald-500'
                  }`}
                />
                <button
                  type="button"
                  ...
                  {PW_RULES.map(rule => {
                    ...
                    return (
                      <li key={rule.label} className={`flex items-center gap-2 text-[11px] font-medium ${ok ? 'text-emerald-600' : 'text-red-500'}`}>
                        <span className={`w-3.5 h-3.5 rounded-full flex items-center justify-center shrink-0 ${ok ? 'bg-emerald-100' : 'bg-red-100'}`}>
                          {ok ? '✓' : '✗'}
                        </span>
                        {rule.label}
                      </li>
