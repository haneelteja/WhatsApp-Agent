...
/** Run sentiment detection and return the result without storing. */
export async function detectSentimentText(messageText: string): Promise<ContactSentiment> {
  if (!messageText?.trim() || messageText.length < 3) return 'neutral';
  try {
    ...
    return VALID_SENTIMENTS.includes(raw) ? raw : 'neutral';
  } catch {
    return 'neutral';
  }
}
...
export async function detectAndStoreSentiment(
  ...
  try {
    const sentiment = await detectSentimentText(messageText);
